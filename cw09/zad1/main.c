...
pthread_mutex_t carriage_wait = PTHREAD_MUTEX_INITIALIZER;
pthread_cond_t carriage_order = PTHREAD_COND_INITIALIZER;

pthread_mutex_t wait_for_passanger = PTHREAD_MUTEX_INITIALIZER;
...
void *carriage_routine(void* arg){
    ...
    for(int i = 0; i < routes; ++i){
        pthread_mutex_lock(&carriage_wait);

        while(correct_cariage_id != id && all_loaded != 0){
            pthread_cond_wait(&carriage_order, &carriage_wait);
        }
        ...
        pthread_mutex_lock(&wait_for_passanger);
        for(int j = 0; j < carriage_capacity; ++j){
            ...
            while(current_passanger != -1){
                pthread_cond_wait(&current_passanger_change, &wait_for_passanger);
            }  
        }
        pthread_mutex_unlock(&wait_for_passanger);
        ...
        pthread_mutex_unlock(&carriage_wait);
...
int main(int argc, char** argv){
    ...
    for(int i = 0; i < passengers_count; ++i){
        ...
        if(pthread_create(&passengers_keys[i], NULL, passenger_routine, &passengers_id[i]) != 0){
    ...
    for(int i = 0; i < carriages_count; ++i){
        carriages_id[i] = i;
        if(pthread_create(&carriages_keys[i], NULL, carriage_routine, &carriages_id[i]) != 0){
    ...
    pthread_mutex_destroy(&carriage_wait);
    pthread_mutex_destroy(&wait_for_passanger);
